...
We had to remove one entire aphid line from the loaded raw data, as we realized with sequencing that this line was not the one we expected but was identical to another one (TR 619).
...
order.wasps <- data %>%
  select(Wasp_host_species, Parasitoid_line) %>%
  distinct() %>%
  arrange(Wasp_host_species, Parasitoid_line) %>%
  pull(Parasitoid_line)


order.aphids <- data %>%
  select(Hamiltonella_host_species, Aphid_line) %>%
  distinct() %>%
  group_by(Hamiltonella_host_species, Aphid_line) %>%
  pull(Aphid_line)
...
data <- data %>%
  mutate(
    ...
    Parasitoid_line = factor(Parasitoid_line, levels = order.wasps),
    Aphid_line = factor(Aphid_line, levels = order.aphids)
  ) %>%
  arrange(Parasitoid_line, Aphid_line, Hamiltonella_host_species)
```
...
bubbles <- ggplot(data = NULL, aes(Parasitoid_line, Aphid_line)) +
  ...
  scale_x_discrete(limits = levels(obsmeans$Parasitoid_line)) + ## reorder axes according to levels of factor
  scale_y_discrete(labels = levels(obsmeans$Aphid_line)) +
...
bubbles_genotype <- ggplot(data = NULL, aes(Parasitoid_line, Aphid_line)) +
  ...
  scale_x_discrete(
    limits = c(
      levels(obsmeans2$Parasitoid_line)[1:4], "",
      levels(obsmeans2$Parasitoid_line)[5:7], "",
      levels(obsmeans2$Parasitoid_line)[8:13], "", "",
      levels(obsmeans2$Parasitoid_line)[14:35]
    ),
    labels = c(
      levels(obsmeans2$Parasitoid_line)[1:4], "",
      levels(obsmeans2$Parasitoid_line)[5:7], "",
  ...
  scale_y_discrete(labels = levels(obsmeans2$Aphid_line), expand = c(0, 1)) +
  labs(title = NULL, x = "Parasitoid line", y = "Aphid line") +
  theme(legend.position = "none") +
...
if (file.exists(here("R_output", "Parasitism_all_lines_mod_betabinom.pdf"))) {
  plot_v1 + scale_color_manual(values = rep(
    c(
      "black",
      "#ffc266",
      "#e68a00",
  ...
  plot_v1 + scale_color_manual(values = c(
    "black",
    "#ffc266",
    "#e68a00",
    "#995c00",
...
```{r plot_v2}
obsmeans <- obsmeans %>%
  mutate(Origin_ham = factor(Origin_ham,
    levels = c(
      "none",
      "*A. urticata*",
      "*A. ruborum*",
...
if (
  ...
  plot_v2 +
    scale_color_manual(values = c("#e68a00", "#53c71a", "#00a0f0", "#5c0aff")) +
    theme(legend.position = "none")
} else {
  pdf(
    height = 5, width = 4,
  ...
  plot_v2 +
    scale_color_manual(
      values = c("black", "#e68a00", "#53c71a", "#00a0f0", "#5c0aff")
    ) +
    theme(legend.position = "none")
  dev.off()
}
...
fits_scaled <- data %>%
  ...
  mutate(Origin_ham2 = factor(
    Origin_ham2,
    c(
      "*H. defensa* original host: *A. hederae*",
      "*H. defensa* original host: *A. fabae fabae*",
      "*H. defensa* original host: *A. ruborum*",
      "*H. defensa* original host: *A. urticata*",
      "*H. defensa* original host: none"
    )
  )) %>%
  arrange(Origin_para) %>%
  arrange(Origin_ham2)
...
if (
  file.exists(
    here("R_output", "Relative_parasitism_all_lines_modOLRE.pdf")
  )
) {
  plot_scaled +
  ...
  pdf(
    height = 4, width = 4,
    file = here("R_output", "Relative_parasitism_all_lines_modOLRE.pdf")
  )
  plot_scaled +
    scale_fill_manual(values = c("#e68a00", "#53c71a", "#00a0f0", "#5c0aff")) +
    theme(legend.position = "none")
...
if (
  file.exists(
    here("R_output", "Relative_parasitism_all_lines_by_ham_modOLRE.pdf")
  )
) {
  plot_scaled2 +
    scale_fill_manual(values = c("#e68a00", "#53c71a", "#00a0f0", "#5c0aff")) +
  ...
  pdf(
    height = 6, width = 4,
    file = here("R_output", "Relative_parasitism_all_lines_by_ham_modOLRE.pdf")
  )
  plot_scaled2 +
    scale_fill_manual(values = c("#e68a00", "#53c71a", "#00a0f0", "#5c0aff")) +
    theme(legend.position = "none")
...
fits_logit %>%
  mutate(Ham_type = case_when(
    Hamiltonella_host_species == "none" ~ "none",
