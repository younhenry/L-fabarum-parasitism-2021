...
pacman::p_load(cmdstanr, brms, Rmisc, tidyverse, patchwork, ggplot2, ggtext, bayesplot, tidybayes, kableExtra, viridis, boot)
...
# I removed one entire transfected line as we realized with sequencing this line was not the one we expected but was identical to another one (TR 619)
...
order.genotypes=as.factor(c(12,11,3,15,18,19,16,17,14,13,6,8,7,1,9,2,10,5,4))
...
mean_data$Wasp_genotype=subset(data, data$Replicate==2)$Wasp_genotype
...
order.wasps.genotype=c("Yp20-027", "Yp20-004", "Yp20-058", "Yp20-075", "Yp20-081", "S251", "Yp20-070", "Yp20-003", "Yp20-021", "Yp20-044", "Yp20-055", "Yp20-072", "Yp20-080", "Yp20-038", "Yp20-006", "Yp20-013", "Yp20-035", "Yp20-009", "Yp20-079", "Yp20-037", "Yp20-050", "Yp20-060", "Yp20-068", "Yp20-084", "F316",     "G339",     "G363",     "G392",     "S350",     "Yp20-032", "Yp20-002", "Yp20-033", "Yp20-048", "Yp20-063", "Yp20-071")

mean_data2=mean_data %>%
  mutate(Parasitoid_line = factor(Parasitoid_line, levels = order.wasps.genotype)) %>%
  mutate(Wasp_genotype = factor(Wasp_genotype, levels = order.genotypes)) %>%
  arrange(Parasitoid_line) %>%
  arrange(Wasp_genotype)

  mean_data2$niche<-ifelse(mean_data2$Wasp_genotype=="12"| mean_data2$Wasp_genotype=="3" | mean_data2$Wasp_genotype=="2" , "generalist", "specialist")

mean_data2=mean_data2 %>%
  mutate(Wasp_host_species = factor(Wasp_host_species, levels = order.origin))%>%
  mutate(Wasp_genotype = factor(Wasp_genotype, levels = order.genotypes))%>%
  mutate(niche = factor(niche, levels = c("generalist", "specialist")))%>%
...
ggSize=0.25
  ...
  geom_point(data=mean_data2, aes(size=Parasitism_success.mean*25, color=Wasp_host_species, shape=16))+ ##adjust size to frequency
  scale_shape_identity()+
  scale_colour_manual(values=colors2)+ ##define custom colors for protected and unprotected
  geom_text (data=mean_data2[mean_data2$Parasitism_success.mean>=0.01,], aes(label=round(Parasitism_success.mean*100,0)), size=3.5)+ 
  geom_text(data=mean_data2, aes(label = Wasp_genotype, x = Parasitoid_line, y = 9.8), size=3.5) +  
  scale_size_identity()+ ##do not plot zero frequency
  scale_x_discrete(limits = c(levels(mean_data2$Parasitoid_line)[1:4], "", levels(mean_data2$Parasitoid_line)[5:7], "", levels(mean_data2$Parasitoid_line)[8:13], "", "", levels(mean_data2$Parasitoid_line)[14:35]), labels=c(levels(mean_data2$Parasitoid_line)[1:4], "", levels(mean_data2$Parasitoid_line)[5:7], "", levels(mean_data2$Parasitoid_line)[8:13], "", "", levels(mean_data2$Parasitoid_line)[14:35]))+
  ...
  theme(panel.background = element_blank(),
        ...
        axis.text.x=element_text(angle=90,size=10,colour='black', vjust=0.4, hjust=1),
        axis.text.y=element_text(size=10,colour='black'),
        axis.ticks=element_line(size=ggSize), 
        axis.line = element_line(size =ggSize, colour='black'))
...
data<-data %>% 
  ...
  mutate(Origin_para2=factor(paste("Parasitoid original host: *",Origin_para,"*",sep="")))%>%
  ...
  mutate(Origin_ham2=factor(paste("*H. defensa* original host: ",Origin_ham,"",sep="")))%>%
...
```{r trials mods with OLRE}

#########################################################################################
########################  to suppress !!!! ##############################################
#########################################################################################

#### Test section to select models

modOLRE1 <- brm(
  bf(Mummies_amount|trials(Nymphs_corrected)~
       0 + Wasp_host_species + Wasp_host_species:Hamiltonella_host_species + Hamiltonella_host_species + Wasp_genotype + Wasp_genotype:Wasp_host_species +## origin in the wild are fixed effects
       (Aphid_line|Wasp_genotype)+ ## hamiltonella lines have a random effect nested by parasitoid genotype
       (1|replicate_fullID)+ ## random effect of the replicate inside each aphid line (not changing much)
       (1|observation), ## this is the OLRE
     family="binomial"),
  data=data,
  chains=4, iter=1000, seed=666,
  prior=prior,
...
pp_check(modOLRE1,"stat_2d")
pp_check(modOLRE2,"stat_2d")
loo_compare(add_criterion(modOLRE1, c("loo")), add_criterion(modOLRE2, c("loo")))

fits<-data %>% 
  select(Hamiltonella_host_species,Wasp_host_species,
         Origin_ham,Origin_para,Origin_para2, Wasp_genotype, Origin_ham2) %>% 
  distinct() %>% 
  mutate(Nymphs_corrected=1) %>% 
  add_fitted_draws(modOLRE1,re_formula = NA) %>%
  ungroup()
...
modOLRE <- brm(
  bf(Mummies_amount|trials(Nymphs_corrected)~
       0+Wasp_host_species+Wasp_host_species:Hamiltonella_host_species+Hamiltonella_host_species+ ## origin in the wild are fixed effects
       (Aphid_line|Wasp_genotype)+ ## hamiltonella lines have a random effect nested by parasitoid genotype
       (1|replicate_fullID)+ ## random effect of the replicate inside each aphid line (not changing much)
       (1|observation), ## this is the OLRE
     family="binomial"),
...
plot_v1<-ggplot(fits)+
  geom_boxplot(data=data,
              aes(Origin_para,
                  ...
                  color= Aphid_line),
              alpha=0.3)+
  stat_eye(aes(Origin_para,.value),
           normalize="xy",
...
plot_scaled_probs<-ggplot(fits_scaled_probs)+
  geom_jitter(data=subset(obsmeans, obsmeans$Hamiltonella_host_species!="none"),
              aes(Origin_para,
                  relative_mean_obs, color=Origin_para),
              height=0,width=0.2,
              size=2,alpha=0.7)+
  stat_eye(aes(Origin_para,.value,fill=Origin_para),
           normalize="xy",
